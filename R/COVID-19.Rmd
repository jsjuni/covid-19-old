...
```{r echo = FALSE}
covid_st <- read.csv("~/COVID-19/nytimes/covid-19-data/us-states.csv")
covid_st$date <- as.Date(covid_st$date)

covid_co <- read.csv("~/COVID-19/nytimes/covid-19-data/us-counties.csv")
covid_co$date <- as.Date(covid_co$date)

pop_co <- read.csv("~/COVID-19/census/co-est2019-alldata.csv")[, c("STATE", "COUNTY", "STNAME", "CTYNAME", "POPESTIMATE2019")]
pop_st <- read.csv("~/COVID-19/census/SCPRC-EST2019-18+POP-RES.csv")[, c("STATE", "NAME", "POPESTIMATE2019")]

# https://gist.github.com/dantonnoriega/bf1acd2290e15b91e6710b6fd3be0a53

postal <- read.csv("~/COVID-19/dantonnoriega/us-state-ansi-fips.csv")
...
m_st$w_ago <- m_st$date - 7
m_st <- merge(x=m_st, y=m_st[, c("date", "fips", "cases", "deaths")], by.x=c("fips", "w_ago"), by.y=c("fips", "date"), all.x=TRUE, suffixes=c("", ".lw"))
...
m_st$new_cases <- m_st$cases - m_st$cases.lw
m_st$new_deaths <- m_st$deaths - m_st$deaths.lw
...
print(t_st[c_order_st, c("state", "cases", "cases_permil", "c_rank")])
```
...
print(t_st[d_order_st, c("state", "deaths", "deaths_permil", "d_rank")])
```
...
print(t_st[new_c_order_st, c("state", "new_cases", "new_cases_permil", "new_c_rank")])
```
...
print(t_st[new_d_order_st, c("state", "new_deaths", "new_deaths_permil", "new_d_rank")])
```
